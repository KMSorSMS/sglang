#!/bin/bash
# Mooncake 一体化构建脚本：准备源码 → 安装依赖 → 编译 → 打包 wheel → 验证
# 用法: build_mooncake.sh <MCAKE_UPPER_PATH> [MCAKE_OPTION] [MCAKE_CLEAR]
#   MCAKE_UPPER_PATH : Mooncake 代码的上级目录（clone 模式下使用）。
#   MCAKE_OPTION     : 编译模式，"te"(默认，含 CUDA/EP/NVLink) 或 "store"(纯 store，无 CUDA/EP)。
#   MCAKE_CLEAR      : 编译后是否清理源码，1=清理(默认) 0=保留。
# 从 JD-v0.5.17 起，当 MOONCAKE_LOCAL_SOURCE_DIR 指向本地预置源码树时直接原地编译，
# 不再从 coding 平台 clone，也不改写 .gitmodules 子模块 URL。
# MOONCAKE_EXPECTED_REVISION 语义为版本号：校验源码树 pyproject.toml version 与基础镜像一致。
set -euo pipefail
...
# 本地预置源码树路径。非空且存在时原地编译，跳过 coding clone 和 submodule update。
MOONCAKE_LOCAL_SOURCE_DIR="${MOONCAKE_LOCAL_SOURCE_DIR:-}"
if [[ -n "${MOONCAKE_LOCAL_SOURCE_DIR}" && -d "${MOONCAKE_LOCAL_SOURCE_DIR}" ]]; then
    MCAKE_PATH="${MOONCAKE_LOCAL_SOURCE_DIR}"
    MCAKE_UPPER_PATH="$(cd "${MCAKE_PATH}/.." && pwd)"
    MCAKE_CLEAR="0"
fi
...
MOONCAKE_EXPECTED_REVISION="${MOONCAKE_EXPECTED_REVISION:?exact Mooncake revision is required}"
MCAKE_COMMIT=""
MCAKE_SUBMODULES=""
YAML_CPP_DIR=""
YAML_CPP_DIR_ORIGIN=""
...
configure_cmake_args() {
    ...
    if [[ -n "${configured_yaml_cpp_dir}" ]]; then
        YAML_CPP_DIR="${configured_yaml_cpp_dir}"
...
cache_cmake_args() {
    ...
    local stable_args=()
    for arg in "${CMAKE_ARGS[@]}"; do
        # This discovered absolute path is useful audit metadata, but it is not
        # a portable build identity across otherwise identical containers.
        if [[ "${YAML_CPP_DIR_ORIGIN}" == "auto" \
            && "${arg}" == "-Dyaml-cpp_DIR=${YAML_CPP_DIR}" ]]; then
            continue
        fi
        stable_args+=("${arg}")
    done
...
install_cached_mooncake_wheel_if_available() {
    ...
    local cached_option
    local cached_python
    local cached_cmake_args
    local cached_yaml_cpp_dir
    local cached_yaml_cpp_origin
    local expected_yaml_cpp_origin
    local wheel_name
    local wheel_sha256
    local actual_wheel_sha256
    ensure_python_context
    if [[ "${MCAKE_OPTION}" != "store" ]]; then
        # Match the exact CMake argument set used by the source-build path. In
        # particular, CUDA stub linker flags are part of the cache provenance.
    ...
    build_info="${cache_path}/build_info.txt"
    if [[ ! -f "${build_info}" ]]; then
        if [[ "${MOONCAKE_REQUIRE_CACHE}" == "1" ]]; then
            print_error "Mooncake wheel cache metadata missing: ${build_info}"
        fi
        echo "[Mooncake CI] Mooncake cache metadata missing，继续源码编译"
        return 1
    fi
    cached_branch=$(sed -n 's/^MCAKE_BRANCH=//p' "${build_info}" | head -n 1)
    cached_commit=$(sed -n 's/^MCAKE_COMMIT=//p' "${build_info}" | head -n 1)
    cached_version=$(sed -n 's/^MCAKE_VERSION=//p' "${build_info}" | head -n 1)
    MCAKE_SUBMODULES=$(sed -n 's/^MCAKE_SUBMODULES=//p' "${build_info}" | head -n 1)
    cached_option=$(sed -n 's/^MCAKE_OPTION=//p' "${build_info}" | head -n 1)
    cached_python=$(sed -n 's/^PYTHON_VERSION=//p' "${build_info}" | head -n 1)
    cached_cmake_args=$(sed -n 's/^CMAKE_ARGS=//p' "${build_info}" | head -n 1)
    cached_yaml_cpp_dir=$(sed -n 's/^YAML_CPP_DIR=//p' "${build_info}" | head -n 1)
    cached_yaml_cpp_origin=$(sed -n 's/^YAML_CPP_DIR_ORIGIN=//p' "${build_info}" | head -n 1)
    wheel_name=$(sed -n 's/^WHEEL=//p' "${build_info}" | head -n 1)
    wheel_sha256=$(sed -n 's/^WHEEL_SHA256=//p' "${build_info}" | head -n 1)
    expected_yaml_cpp_origin="${YAML_CPP_DIR_ORIGIN:-auto}"
    # 缓存命中校验以版本号为准（MOONCAKE_EXPECTED_REVISION 现在是版本号，不再是 commit SHA）。
    # 对老缓存（无 MCAKE_VERSION 行）回退到 branch 比较。
    if [[ -z "${cached_version}" && "${cached_branch}" != "${MCAKE_BRANCH}" ]] \
        || [[ -n "${cached_version}" && "${cached_version}" != "${MCAKE_VERSION}" ]] \
        || "${cached_option}" != "${MCAKE_OPTION}" \
        || "${cached_python}" != "${PYTHON_VERSION}" \
        || "${cached_cmake_args}" != "$(cache_cmake_args)" \
        || -z "${cached_yaml_cpp_dir}" \
        || "${cached_yaml_cpp_origin}" != "${expected_yaml_cpp_origin}" \
        || ( "${cached_yaml_cpp_origin}" == "explicit" \
            && "${cached_yaml_cpp_dir}" != "${YAML_CPP_DIR}" ) \
        || -z "${MCAKE_SUBMODULES}" ]]; then
        print_error "cached Mooncake revision mismatch or build provenance mismatch: ${build_info}"
    fi
    MCAKE_COMMIT="${cached_commit}"
    YAML_CPP_DIR="${cached_yaml_cpp_dir}"
    YAML_CPP_DIR_ORIGIN="${cached_yaml_cpp_origin}"
    whl_file="${cache_path}/${wheel_name}"
    if [[ ! -f "${whl_file}" ]]; then
        print_error "cached Mooncake metadata wheel missing: ${whl_file}"
    fi
    actual_wheel_sha256=$(sha256sum "${whl_file}" | awk '{print $1}')
    if [[ "${actual_wheel_sha256}" != "${wheel_sha256}" ]]; then
        print_error "cached Mooncake wheel sha256 mismatch: ${whl_file}"
    fi

    print_section "Installing Mooncake wheel from cache"
    echo "[Mooncake CI] Cached wheel: ${whl_file}, sha256=${actual_wheel_sha256}"
    echo "[Mooncake CI] Cached source: branch=${cached_branch}, commit=${cached_commit}, submodules=${MCAKE_SUBMODULES}"
    echo "[Mooncake CI] Cached build contract: cmake=$(cache_cmake_args), yaml-cpp_DIR=${YAML_CPP_DIR}"
...
save_mooncake_wheel_cache() {
    ...
    mkdir -p "${cache_path}"
    cached_wheel="${cache_path}/$(basename "${whl_file}")"
    (
        flock 8
        rm -f "${cache_path}"/mooncake_transfer_engine*.whl
        cp -f "${whl_file}" "${cache_path}/"
        wheel_sha256=$(sha256sum "${cached_wheel}" | awk '{print $1}')
        cat > "${cache_path}/build_info.txt.tmp.$$" <<EOF
...
CMAKE_ARGS=$(cache_cmake_args)
YAML_CPP_DIR=${YAML_CPP_DIR}
YAML_CPP_DIR_ORIGIN=${YAML_CPP_DIR_ORIGIN}
WHEEL=$(basename "${cached_wheel}")
WHEEL_SHA256=${wheel_sha256}
BUILT_AT=$(date -Iseconds)
EOF
        mv -f "${cache_path}/build_info.txt.tmp.$$" "${cache_path}/build_info.txt"
    ) 8>"${lock_file}"

    print_success "Mooncake wheel cached: ${cached_wheel}"
}
...
save_mooncake_engine_cache() {
    ...
    cat > "${MOONCAKE_ENGINE_CACHE_DIR}/engine_info.txt" <<EOF
MCAKE_BRANCH=${MCAKE_BRANCH}
MCAKE_COMMIT=${MCAKE_COMMIT}
MCAKE_SUBMODULES=${MCAKE_SUBMODULES}
MCAKE_OPTION=${MCAKE_OPTION}
...
clear_history() {
    if [[ -n "${MOONCAKE_LOCAL_SOURCE_DIR}" ]]; then
        echo "[Mooncake CI] 本地源码树模式，跳过 clear_history"
        return 0
    fi
...
# 读取本地源码树 mooncake-wheel/pyproject.toml 的 version 字段。
read_local_source_version() {
    local pyproject="${MCAKE_PATH}/mooncake-wheel/pyproject.toml"
    if [[ ! -f "${pyproject}" ]]; then
        print_error "本地源码树缺少 mooncake-wheel/pyproject.toml: ${pyproject}"
    fi
    sed -n 's/^version[[:space:]]*=[[:space:]]*"\(.*\)"/\1/p' "${pyproject}" | head -1
}

# 本地预置源码树模式：原地编译，不 clone、不 update submodule、不改 .gitmodules。
# 仅校验源码树版本与基础镜像安装的 mooncake-transfer-engine 版本一致。
prepare_local_source() {
    if [[ ! -d "${MOONCAKE_LOCAL_SOURCE_DIR}" ]]; then
        print_error "MOONCAKE_LOCAL_SOURCE_DIR 不存在: ${MOONCAKE_LOCAL_SOURCE_DIR}"
    fi

    print_section "Preparing Mooncake local source tree"
    ...
    if [[ -z "${source_version}" ]]; then
        print_error "无法从 mooncake-wheel/pyproject.toml 读取 version"
    fi
    if [[ "${source_version}" != "${MCAKE_VERSION}" ]]; then
        print_error "本地源码树版本(${source_version})与基础镜像 mooncake-transfer-engine(${MCAKE_VERSION})不匹配"
    fi
    echo "[Mooncake CI] 本地源码树版本校验通过: ${source_version}"

    MCAKE_COMMIT=$(git -C "${MCAKE_PATH}" rev-parse HEAD 2>/dev/null || echo "<non-git>")
    MCAKE_SUBMODULES=$(git -C "${MCAKE_PATH}" submodule status --recursive 2>/dev/null | paste -sd ';' - || echo "")
    echo "[Mooncake CI] Mooncake local source commit: ${MCAKE_COMMIT}"
    echo "[Mooncake CI] MCAKE_SUBMODULES=${MCAKE_SUBMODULES}"

    print_section "Mooncake local source tree ready (原地编译，跳过 submodule update)"
}
...
pull_code() {
    ...
    echo "git clone Mooncake $MCAKE_REPO_URL branch=$MCAKE_BRANCH without checkout..."
    if ! git clone -q --no-checkout -b "$MCAKE_BRANCH" "$MCAKE_REPO_URL"; then
        print_error "Failed to git clone Mooncake branch=${MCAKE_BRANCH}"
    fi

    cd "$MCAKE_PATH"
    check_success "Failed to change to $MCAKE_PATH directory"

    MCAKE_BRANCH_REF="refs/remotes/origin/${MCAKE_BRANCH}"
    if ! git cat-file -e "${MOONCAKE_EXPECTED_REVISION}^{commit}"; then
        print_error "Mooncake pinned revision is unavailable: ${MOONCAKE_EXPECTED_REVISION}"
    fi
    if ! git merge-base --is-ancestor "${MOONCAKE_EXPECTED_REVISION}" "${MCAKE_BRANCH_REF}"; then
        print_error "Mooncake pinned revision ${MOONCAKE_EXPECTED_REVISION} does not belong to ${MCAKE_BRANCH_REF}"
    fi
    if ! git checkout --quiet --detach "${MOONCAKE_EXPECTED_REVISION}"; then
        print_error "Failed to checkout pinned Mooncake revision: ${MOONCAKE_EXPECTED_REVISION}"
    fi

    MCAKE_COMMIT=$(git rev-parse HEAD)
    if [[ "${MCAKE_COMMIT}" != "${MOONCAKE_EXPECTED_REVISION}" ]]; then
        print_error "Mooncake source revision mismatch: expected=${MOONCAKE_EXPECTED_REVISION}, actual=${MCAKE_COMMIT}"
    fi
    echo "[Mooncake CI] Mooncake branch: $(git rev-parse --abbrev-ref HEAD), commit: ${MCAKE_COMMIT}"
    ...
    if [ -f "${MCAKE_PATH}/.gitmodules" ]; then
        ...
        MCAKE_SUBMODULES=$(git submodule status --recursive | paste -sd ';' -)
        if [[ -z "${MCAKE_SUBMODULES}" ]]; then
            print_error "Mooncake recursive submodule provenance is empty"
        fi
        echo "[Mooncake CI] MCAKE_SUBMODULES=${MCAKE_SUBMODULES}"
    else
...
ensure_yaml_cpp_dev() {
    ...
    if [[ -n "${configured_yaml_cpp_dir}" ]]; then
        YAML_CPP_DIR="${configured_yaml_cpp_dir}"
        YAML_CPP_DIR_ORIGIN="explicit"
        echo "[Mooncake CI] yaml-cpp_DIR=${YAML_CPP_DIR} (explicit)"
        return
    fi
...
verify_update_wheel_version() {
    ...
    if [[ -f "${wheel_source}" ]]; then
        whl_file="${wheel_source}"
    else
        whl_file=$(find "${wheel_source}" -name "mooncake_transfer_engine*.whl" 2>/dev/null | head -1)
    fi
    if [ -z "$whl_file" ]; then
        print_error "No mooncake_transfer_engine wheel found in $wheel_source"
    fi
...
compile_and_package() {
    ...
    if [ "$MCAKE_CLEAR" != "0" ] && [[ -z "${MOONCAKE_LOCAL_SOURCE_DIR}" ]]; then
        print_section "Delete Mooncake Compile files"
        cd "$MCAKE_UPPER_PATH"
        check_success "Failed to change to $MCAKE_UPPER_PATH directory"
        rm -rf "$MCAKE_REPO_DIR"
        print_success "Delete Mooncake Compile files succeed"
    elif [[ -n "${MOONCAKE_LOCAL_SOURCE_DIR}" ]]; then
        echo "[Mooncake CI] 本地源码树模式，跳过删除源码目录"
    fi
}
...
echo "[Mooncake CI] MOONCAKE_EXPECTED_REVISION=${MOONCAKE_EXPECTED_REVISION}"
echo "[Mooncake CI] MOONCAKE_LOCAL_SOURCE_DIR=${MOONCAKE_LOCAL_SOURCE_DIR:-<unset>}"
echo "[Mooncake CI] ======================================"
