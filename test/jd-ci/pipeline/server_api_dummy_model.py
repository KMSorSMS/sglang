...
def _invalid_thinking(
    context: RequestContext, case_id: str, value: Any
) -> dict[str, Any]:
    ...
    try:
        ...
        try:
            process = popen_launch_server(
                config.model_path,
                DEFAULT_URL_FOR_TEST,
                timeout=min(config.timeout_seconds, DEFAULT_TIMEOUT_FOR_SERVER_LAUNCH),
            ...
            results.extend(
                _blocked_result(case, startup_error) for case in SERVER_CASES
            )
        else:
