...
执行模式（互斥）:
  -r, --review       正常 PR 评审；默认模式。
                     强制编译 SGL-Kernel、Mooncake 和 hpc-ops，更新正式缓存，
...
if [[ "${CI_MODE}" == "merge" ]]; then
    if [[ -n "${JD_CI_RELEASE_ARTIFACT_REF:-}" ]]; then
        if [[ "${JD_CI_RELEASE_ARTIFACT_REF}" != refs/* ]]; then
            usage_error "JD_CI_RELEASE_ARTIFACT_REF 必须是完整 refs/... 引用"
        fi
        if ! git -C "${SOURCE_PATH}" rev-parse --verify --quiet \
            "${JD_CI_RELEASE_ARTIFACT_REF}^{commit}" >/dev/null; then
            usage_error "无法解析显式正式 ref ${JD_CI_RELEASE_ARTIFACT_REF}"
        fi
        SGL_KERNEL_RELEASE_REF="${JD_CI_RELEASE_ARTIFACT_REF}"
    else
        local_release_ref="refs/heads/${RELEASE_ARTIFACT_BRANCH}"
        remote_release_ref="refs/remotes/origin/${RELEASE_ARTIFACT_BRANCH}"
        local_release_commit=$(git -C "${SOURCE_PATH}" rev-parse --verify --quiet \
            "${local_release_ref}^{commit}" || true)
        remote_release_commit=$(git -C "${SOURCE_PATH}" rev-parse --verify --quiet \
            "${remote_release_ref}^{commit}" || true)
        if [[ -n "${local_release_commit}" && -n "${remote_release_commit}" \
            && "${local_release_commit}" != "${remote_release_commit}" ]]; then
            usage_error "local and remote release refs disagree; set JD_CI_RELEASE_ARTIFACT_REF to the approved full ref"
        fi
        set +e
        live_release_output=$(git -C "${SOURCE_PATH}" ls-remote --exit-code origin \
            "refs/heads/${RELEASE_ARTIFACT_BRANCH}" 2>/dev/null)
        live_release_status=$?
        set -e
        if [[ ${live_release_status} -eq 0 ]]; then
            live_release_commit=$(awk 'NR == 1 {print $1}' <<< "${live_release_output}")
        elif [[ ${live_release_status} -eq 2 ]]; then
            live_release_commit=""
        else
            usage_error "cannot verify live release ref; set JD_CI_RELEASE_ARTIFACT_REF to the approved full ref"
        fi
        if [[ -n "${remote_release_commit}" \
            && "${remote_release_commit}" != "${live_release_commit}" ]]; then
            usage_error "remote-tracking release ref is stale; fetch it or set JD_CI_RELEASE_ARTIFACT_REF"
        fi
        if [[ -n "${local_release_commit}" && -n "${live_release_commit}" \
            && "${local_release_commit}" != "${live_release_commit}" ]]; then
            usage_error "local release ref disagrees with live origin; set JD_CI_RELEASE_ARTIFACT_REF"
        fi
        if [[ -n "${remote_release_commit}" ]]; then
            SGL_KERNEL_RELEASE_REF="${remote_release_ref}"
        elif [[ -n "${local_release_commit}" ]]; then
            SGL_KERNEL_RELEASE_REF="${local_release_ref}"
        fi
    fi
    if [[ -z "${SGL_KERNEL_RELEASE_REF}" ]]; then
        usage_error "无法解析 ${RELEASE_ARTIFACT_BRANCH} 的正式完整 ref；可设置 JD_CI_RELEASE_ARTIFACT_REF"
    fi
    SGL_KERNEL_EXPECTED_SOURCE_COMMIT=$(git -C "${SOURCE_PATH}" rev-parse "${SGL_KERNEL_RELEASE_REF}^{commit}")
    SGL_KERNEL_EXPECTED_TREE=$(git -C "${SOURCE_PATH}" rev-parse "${SGL_KERNEL_RELEASE_REF}:python/sglang/kernels/aot")
fi
echo "[SGLang CI] SGL_KERNEL_RELEASE_REF=${SGL_KERNEL_RELEASE_REF:-<not-required>}"
echo "[SGLang CI] SGL_KERNEL_EXPECTED_SOURCE_COMMIT=${SGL_KERNEL_EXPECTED_SOURCE_COMMIT:-<not-required>}"
echo "[SGLang CI] SGL_KERNEL_EXPECTED_TREE=${SGL_KERNEL_EXPECTED_TREE:-<not-required>}"
...
# 从 JD-v0.5.17 起，Mooncake 不再从 coding 平台 clone，而是使用本地预置源码树编译。
# MOONCAKE_EXPECTED_REVISION 的语义从 commit SHA 改为版本号：build_mooncake.sh 据此
# 校验本地源码树 mooncake-wheel/pyproject.toml 的 version 与基础镜像安装版本一致。
...
HPC_OPS_SOURCE_HOST="${HPC_OPS_SOURCE_HOST:-${CI_WORK_DIR}/ci/hpc-ops}"
HPC_OPS_EXPECTED_REVISION="${HPC_OPS_EXPECTED_REVISION:-$(git -C "${HPC_OPS_SOURCE_HOST}" rev-parse HEAD 2>/dev/null || true)}"
HPC_OPS_SOURCE_REVISION="<source-not-required>"
PERSISTENT_HPC_OPS_CACHE_HOST="${CI_WORK_DIR}/ci/sglang/hpc-ops/${BASE_IMAGE_TAG}"
case "${CI_MODE}" in
    review|merge)
    ...
    temp-image)
        ...
        HPC_OPS_WHEEL_CACHE_HOST="${JD_CI_TEMP_ARTIFACT_ROOT}/hpc-ops/${BASE_IMAGE_TAG}"
        ...
        if [[ "${JD_CI_SKIP_HPC_OPS_BUILD}" == "1" ]]; then
            HPC_OPS_ARTIFACT_SCOPE="base-image"
        else
            HPC_OPS_ARTIFACT_SCOPE="temporary-build"
        fi
        ;;
esac
...
echo "[SGLang CI] HPC_OPS_ARTIFACT_SCOPE=${HPC_OPS_ARTIFACT_SCOPE}"
...
echo "[SGLang CI] MOONCAKE_EXPECTED_REVISION=${MOONCAKE_EXPECTED_REVISION}"
echo "[SGLang CI] MOONCAKE_TE_CACHE_BASE=${MOONCAKE_ENGINE_CACHE_HOST}"
echo "[SGLang CI] HPC_OPS_SOURCE_HOST=${HPC_OPS_SOURCE_HOST}"
echo "[SGLang CI] HPC_OPS_EXPECTED_REVISION=${HPC_OPS_EXPECTED_REVISION}"
echo "[SGLang CI] HPC_OPS_WHEEL_CACHE_BASE=${HPC_OPS_WHEEL_CACHE_HOST}"

if [[ "${CI_MODE}" == "merge" ]]; then
    MOONCAKE_FORCE_REBUILD=0
    MOONCAKE_REQUIRE_CACHE=1
    HPC_OPS_FORCE_REBUILD=0
    HPC_OPS_REQUIRE_CACHE=1
else
...
echo "[SGLang CI] HPC_OPS_FORCE_REBUILD=${HPC_OPS_FORCE_REBUILD}"
echo "[SGLang CI] HPC_OPS_REQUIRE_CACHE=${HPC_OPS_REQUIRE_CACHE}"
...
HPC_OPS_COMPILE_DIR="${HPC_OPS_WORK_DIR}/compile"
HPC_OPS_INSTALL_DIR="${MAIN_CONTAINER_TMP_DIR}/hpc-ops-install"
...
HPC_OPS_NEEDS_SOURCE=0
if [[ "${CI_MODE}" == "review" ]] \
    || [[ "${CI_MODE}" == "temp-image" && "${JD_CI_SKIP_HPC_OPS_BUILD}" == "0" ]]; then
    HPC_OPS_NEEDS_SOURCE=1
fi

HPC_OPS_DOCKER_SOURCE_ARGS=()
if [[ "${HPC_OPS_NEEDS_SOURCE}" == "1" ]]; then
    if [[ ! -d "${HPC_OPS_SOURCE_HOST}" ]]; then
        echo "[SGLang CI] ERROR: hpc-ops 源码目录不存在: ${HPC_OPS_SOURCE_HOST}" >&2
        exit 1
    fi
    if [[ ! -f "${HPC_OPS_SOURCE_HOST}/Makefile" ]]; then
        echo "[SGLang CI] ERROR: hpc-ops 源码目录中未找到 Makefile: ${HPC_OPS_SOURCE_HOST}" >&2
        exit 1
    fi
    HPC_OPS_SOURCE_REVISION=$(git -C "${HPC_OPS_SOURCE_HOST}" rev-parse HEAD 2>/dev/null || true)
    if [[ "${HPC_OPS_SOURCE_REVISION}" != "${HPC_OPS_EXPECTED_REVISION}" ]]; then
        echo "[SGLang CI] ERROR: hpc-ops 源码 revision 不匹配: expected=${HPC_OPS_EXPECTED_REVISION}, actual=${HPC_OPS_SOURCE_REVISION:-<not-a-git-checkout>}" >&2
        exit 1
    fi
    echo "[SGLang CI] HPC_OPS_SOURCE_REVISION=${HPC_OPS_SOURCE_REVISION}"
    HPC_OPS_DOCKER_SOURCE_ARGS=(-v "${HPC_OPS_SOURCE_HOST}:/hpc-ops-src:ro")
fi
...
HPC_OPS_BUILD_SCRIPT="${SOURCE_PATH}/test/jd-ci/env/build_hpc_ops.sh"
chmod +x "${HPC_OPS_BUILD_SCRIPT}"
...
run_docker_attached docker run \
    ...
    -e HPC_OPS_FORCE_REBUILD="${HPC_OPS_FORCE_REBUILD}" \
    -e HPC_OPS_REQUIRE_CACHE="${HPC_OPS_REQUIRE_CACHE}" \
    -e BASE_IMAGE_TAG="${BASE_IMAGE_TAG}" \
    -e EVENT_TYPE="${EVENT_TYPE}" \
    -e SGL_KERNEL_EXPECTED_SOURCE_COMMIT="${SGL_KERNEL_EXPECTED_SOURCE_COMMIT}" \
    -e SGL_KERNEL_EXPECTED_TREE="${SGL_KERNEL_EXPECTED_TREE}" \
    ...
    -v "${HPC_OPS_WHEEL_CACHE_HOST}:/hpc-ops-wheels" \
    "${HPC_OPS_DOCKER_SOURCE_ARGS[@]}" \
    -v "${SOURCE_PATH}:${SOURCE_PATH}" \
    -w "${SOURCE_PATH}" \
    --entrypoint /bin/bash \
    ...
    -c "
        ...
        # These paths isolate the running CI process only. Keep them out of
        # the container config so the published image preserves the base
        # runtime environment instead of retaining commit-scoped paths.
        export JD_LOG_DIR='${CI_LOGS_DIR}/helpers/sglang'
        export CI_TMP_DIR='${MAIN_CONTAINER_WORK_DIR}'
        export TMPDIR='/tmp'
        export TMP='/tmp'
        export TEMP='/tmp'
        export CUDA_CACHE_PATH='${MAIN_CONTAINER_WORK_DIR}/cuda-cache'
        export SGL_KERNEL_BUILD_TMPDIR='${SGL_KERNEL_WORK_DIR}/tmp'
        export MOONCAKE_TMP_DIR='${MOONCAKE_TE_WORK_DIR}/tmp'
        export HPC_OPS_BUILD_TMPDIR='${HPC_OPS_WORK_DIR}/tmp'
        export UV_CACHE_DIR='${MAIN_CONTAINER_WORK_DIR}/uv-cache'
        export PIP_CACHE_DIR='${MAIN_CONTAINER_WORK_DIR}/pip-cache'
        export XDG_CACHE_HOME='${MAIN_CONTAINER_WORK_DIR}/xdg-cache'
        export TORCH_EXTENSIONS_DIR='${MAIN_CONTAINER_WORK_DIR}/torch-extensions'
        export TRITON_CACHE_DIR='${MAIN_CONTAINER_WORK_DIR}/triton-cache'
        export TORCHINDUCTOR_CACHE_DIR='${MAIN_CONTAINER_WORK_DIR}/torchinductor-cache'
        ...
        # ---------- 共享环境 ----------
        source '${SOURCE_PATH}/test/jd-ci/env/setup_env.sh' \
            '${SOURCE_PATH}' \
            '${BASE_IMAGE_TAG}' \
            '${CI_WORK_DIR}'
        ...
        # ---------- hpc-ops ----------
        if [[ '${JD_CI_SKIP_HPC_OPS_BUILD}' == '1' ]]; then
            echo '[JD CI] 跳过 hpc-ops 编译和安装 (JD_CI_SKIP_HPC_OPS_BUILD=1)'
        else
            echo '[JD CI] 开始 hpc-ops 编译/安装'
            echo '[JD CI] hpc-ops artifact scope: ${HPC_OPS_ARTIFACT_SCOPE}'
            echo '[JD CI] hpc-ops wheel cache: /hpc-ops-wheels'
            set -o pipefail
            run_with_isolated_workspace '${HPC_OPS_WORK_DIR}' \
                bash '${HPC_OPS_BUILD_SCRIPT}' \
                '/hpc-ops-src' \
                '${HPC_OPS_COMPILE_DIR}' \
                '${HPC_OPS_INSTALL_DIR}' \
                '/hpc-ops-wheels' \
                '${HPC_OPS_FORCE_REBUILD}' \
                '${HPC_OPS_REQUIRE_CACHE}' \
                '${HPC_OPS_EXPECTED_REVISION}' \
                2>&1 | tee '${HPC_OPS_BUILD_LOG}'
            set +o pipefail
        fi
