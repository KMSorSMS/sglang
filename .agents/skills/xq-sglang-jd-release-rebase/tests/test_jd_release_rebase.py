...
class TestJDReleaseRebase(unittest.TestCase):
    ...
    def test_prepare_and_commit_ci_creates_one_head_commit(self):
        ...
        (worktree / "test/jd-ci/README.md").write_text(
            "JD CI corrected\n", encoding="utf-8"
        )
        git(worktree, "add", "test/jd-ci/README.md")
        old_ci_head = head
        git(worktree, "checkout", "-q", "--detach", old_ci_head)
        detached_report = self.repo.path / "detached-report.json"
        detached_check = self.run_tool(
            "check",
            "--state-file",
            str(state_file),
            "--output",
        ...
        self.assertEqual(detached_check.returncode, 4)
        self.assertFalse(json.loads(detached_report.read_text())["target_ref_ok"])
        detached = self.run_tool(
            "amend-ci",
            "--state-file",
            str(state_file),
            "--message",
        ...
        self.assertIn("not attached to the recorded target ref", detached.stderr)
        git(worktree, "checkout", "-q", state["new_internal"]["name"])
        git(worktree, "checkout", "-q", "-b", "same-sha-other-branch")
        other_branch = self.run_tool(
            "amend-ci",
            "--state-file",
            str(state_file),
            "--message",
        ...
        self.assertEqual(other_branch.returncode, 2)
        self.assertIn("not attached to the recorded target ref", other_branch.stderr)
        git(worktree, "checkout", "-q", state["new_internal"]["name"])
        amended = self.run_tool(
            "amend-ci",
            "--state-file",
            str(state_file),
            "--message",
        ...
        state = json.loads(state_file.read_text())
        amended_head = git(worktree, "rev-parse", "HEAD").stdout.strip()
        self.assertNotEqual(amended_head, old_ci_head)
        self.assertEqual(state["ci_commit_sha"], amended_head)
        self.assertEqual(state["new_head"], amended_head)
        self.assertEqual(state["mappings"][first_ci], amended_head)
        self.assertEqual(state["mappings"][second_ci], amended_head)
        self.assertEqual(
            git(worktree, "rev-list", "--count", f"{new_upstream}..HEAD").stdout.strip(),
            "2",
        )
        amended_report = self.repo.path / "amended-report.json"
        rechecked = self.run_tool(
            "check", "--state-file", str(state_file), "--output", str(amended_report)
        )
        self.assertEqual(rechecked.returncode, 0, rechecked.stderr)
        amended_audit = json.loads(amended_report.read_text())
        self.assertEqual(amended_audit["jd_ci_commit_count"], 1)
        self.assertTrue(amended_audit["target_ref_ok"])
