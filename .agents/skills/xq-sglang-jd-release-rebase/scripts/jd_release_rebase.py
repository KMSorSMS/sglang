...
def command_amend_ci(args: argparse.Namespace) -> int:
    state = load_json(Path(args.state_file).resolve())
    if state.get("status") != "completed":
        raise RebaseError(
            "state is not ready to amend the final JD CI commit: "
            + str(state.get("status"))
        )
    worktree = Path(state["worktree"])
    head = run_git(worktree, "rev-parse", "HEAD").stdout.strip()
    expected_ref = state["new_internal"]["ref"]
    current_ref = run_git(
        worktree, "symbolic-ref", "--quiet", "HEAD", check=False
    ).stdout.strip()
    expected_ref_head = run_git(
        worktree, "rev-parse", "--verify", expected_ref, check=False
    ).stdout.strip()
    if current_ref != expected_ref or expected_ref_head != head:
        raise RebaseError(
            "worktree HEAD is not attached to the recorded target ref: "
            + expected_ref
        )
    if head != state.get("ci_commit_sha") or head != state.get("new_head"):
        raise RebaseError("HEAD is not the recorded final JD CI commit")
    parent = run_git(worktree, "rev-parse", "HEAD^").stdout.strip()
    if parent != state.get("production_head"):
        raise RebaseError("final JD CI commit is no longer directly after production HEAD")
    status = run_git(worktree, "status", "--porcelain=v1").stdout.splitlines()
    unstaged = [
        line
        for line in status
    ...
    if unstaged:
        raise RebaseError(
            "stage every intended JD CI correction before amend-ci: "
            + ", ".join(unstaged)
        )
    cached = run_git(worktree, "diff", "--cached", "--quiet", check=False)
    if cached.returncode == 0:
        raise RebaseError("final JD CI amendment has no staged changes")
    run_git(worktree, "commit", "--amend", "-m", args.message)
    final_sha = run_git(worktree, "rev-parse", "HEAD").stdout.strip()
    if run_git(worktree, "rev-parse", "--verify", expected_ref).stdout.strip() != final_sha:
        raise RebaseError("target ref did not move with the amended JD CI commit")
    for item in state["deferred_ci_commits"]:
        state["mappings"][item["sha"]] = final_sha
    state["ci_commit_sha"] = final_sha
    state["new_head"] = final_sha
...
def command_check(args: argparse.Namespace) -> int:
    ...
    target_ref_ok = current_ref == expected_ref and expected_ref_head == head
    ...
    report = {
        ...
        "target_ref_head": expected_ref_head,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    amend_ci = subparsers.add_parser(
        "amend-ci",
        help="Amend a staged correction into the recorded final JD CI commit",
    )
    amend_ci.add_argument("--state-file", required=True)
    amend_ci.add_argument("--message", required=True)
    amend_ci.set_defaults(handler=command_amend_ci)
