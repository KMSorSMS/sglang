...
old JD:       refs/remotes/origin/JD-v0.5.15
new upstream: refs/tags/v0.5.16
new JD:       refs/heads/JD-v0.5.16
```
...
```text
/export/zhangyu/ci/sglang/sgl-kernel/v0.5.16/_deps
```
...
python3 .agents/skills/xq-sglang-jd-release-rebase/scripts/jd_release_rebase.py execute \
  --plan /tmp/jd-release-rebase-plan.json \
  --worktree ../sglang-JD-v0.5.16 \
  --state-file .git/jd-release-rebase/JD-v0.5.16.json
```
...
python3 .agents/skills/xq-sglang-jd-release-rebase/scripts/jd_release_rebase.py commit-ci \
  --state-file .git/jd-release-rebase/JD-v0.5.16.json \
  --message "ci: migrate JD CI for v0.5.16"
```

helper 会把所有 deferred old CI SHA 映射到该 commit，并拒绝中间插入的 commit。该 commit 在验证和发布期间始终必须是 `HEAD`。

若 `commit-ci` 后的本地审计发现最终 CI commit 自身仍需修正，禁止追加第二个
commit，也不要手工改写 state。将修正完整加入 Git index 后使用 `amend-ci`；它只在
`HEAD` 仍是已记录的唯一 CI commit、其 parent 仍是 production HEAD 时执行 amend，
并原子更新所有 deferred SHA 映射：

```bash
python3 .agents/skills/xq-sglang-jd-release-rebase/scripts/jd_release_rebase.py amend-ci \
  --state-file .git/jd-release-rebase/JD-v0.5.16.json \
  --message "ci: migrate JD CI for v0.5.16"
```
