...
class TestCollectPRDelta(unittest.TestCase):
    ...
    def test_explicit_final_ci_head_tracking_does_not_require_self_sha(self):
        base = git(self.repo.path, "rev-parse", "HEAD")
        self.repo.write(
            "test/jd-ci/jd_test_manifest.py",
            "INTERNAL_COMMITS = ()\n"
            "CASES = (\n"
            "    JDCase(case_id='ci', commits=(), tracks_ci_head=True),\n"
            ")\n",
        )
        final_ci = self.repo.commit("ci: final JD CI commit")

        default_result = self.run_collector("--base", base)
        tracked_result = self.run_collector(
            "--base", base, "--allow-tracked-ci-head"
        )

        self.assertEqual(default_result.returncode, 0, default_result.stderr)
        self.assertEqual(
            json.loads(default_result.stdout)["uncovered_commits"], [final_ci]
        )
        self.assertEqual(tracked_result.returncode, 0, tracked_result.stderr)
        tracked = json.loads(tracked_result.stdout)
        self.assertEqual(tracked["manifest"]["tracks_ci_head_case_ids"], ["ci"])
        self.assertEqual(tracked["tracked_ci_head"], final_ci)
        self.assertEqual(tracked["uncovered_commits"], [])
