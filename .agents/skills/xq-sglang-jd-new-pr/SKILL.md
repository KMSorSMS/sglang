...
- release rebase 的唯一 final JD CI commit 使用 `tracks_ci_head=true` 时，只有在该
  commit 已经成为 `HEAD` 后，才可用 `collect_pr_delta.py --allow-tracked-ci-head`
  显式完成自引用审计；普通 PR 禁止使用该开关掩盖未映射 HEAD；
