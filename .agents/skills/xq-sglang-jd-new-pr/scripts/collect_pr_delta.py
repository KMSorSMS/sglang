...
def parse_manifest(path: Path, repo_root: Path) -> dict[str, object]:
    ...
    for node in ast.walk(tree):
        ...
        if function_name != "JDCase":
            continue
        case_id = ""
        tracks_ci_head = False
        for keyword in node.keywords:
            if keyword.arg == "commits":
                mapped_commits.update(_literal_strings(keyword.value))
            elif keyword.arg == "case_id":
                values = _literal_strings(keyword.value)
                case_id = values[0] if values else ""
            elif keyword.arg == "tracks_ci_head":
                try:
                    tracks_ci_head = ast.literal_eval(keyword.value) is True
                except (TypeError, ValueError, SyntaxError):
                    tracks_ci_head = False
        if tracks_ci_head and case_id:
            tracks_ci_head_case_ids.add(case_id)
    ...
    return {
        ...
        "tracks_ci_head_case_ids": sorted(tracks_ci_head_case_ids),
    }
...
def collect_delta(
    ...
    manifest: Path,
    *,
    allow_tracked_ci_head: bool = False,
) -> dict[str, object]:
    ...
    tracked_ci_head = (
        head
        if allow_tracked_ci_head and manifest_data["tracks_ci_head_case_ids"]
        else None
    )
    ...
    return {
        ...
        "tracked_ci_head": tracked_ci_head,
        "uncovered_commits": [
            commit["sha"]
            for commit in commits
            if commit["sha"] not in mapped and commit["sha"] != tracked_ci_head
        ],
    }
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--allow-tracked-ci-head",
        action="store_true",
        help=(
            "Treat HEAD as covered by manifest cases with tracks_ci_head=True; "
            "use only when auditing the unique final JD CI commit"
        ),
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        data = collect_delta(
            ...
            args.base,
            Path(args.manifest),
            allow_tracked_ci_head=args.allow_tracked_ci_head,
        )
